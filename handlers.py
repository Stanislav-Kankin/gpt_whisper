...
async def handle_video(message: Message):
    try:
        ...
        await message.answer("Скачал файл, идёт извлечение аудиодорожки.")

        # Извлекаем аудиодорожку
        audio_path = "audio.mp3"
        if extract_audio_from_video(video_path, audio_path):
            logger.info("Аудиодорожка успешно извлечена")
            await message.answer("Извлёк аудиодорожку, идёт транскрибация.")

            # Транскрибация через OpenAI Whisper с временными метками
            transcription = await transcribe_audio(audio_path, return_timestamps=True)
            if transcription.startswith("Ошибка"):
                logger.error(f"Ошибка транскрибации: {transcription}")
                await message.answer(transcription)
                return
            ...
            # Сохраняем транскрипцию в базе данных
            db = SessionLocal()
            user_data = db.query(UserData).filter(UserData.user_id == message.from_user.id).first()
            if user_data:
                user_data.transcription = transcription
            else:
                user_data = UserData(user_id=message.from_user.id, transcription=transcription)
                db.add(user_data)
            db.commit()
            db.close()
            ...
            # Отправляем сообщение с выбором сценария анализа
            await message.answer(
                "Я закончил транскрибацию звонка, ниже ты можешь найти варианты анализа, обращай внимание на актуальность сценария.\n"
                "<u>Выбери сценарий анализа:</u>\n"
                "\n"
                "<b>1. Стандартная квалификация:</b>\n"
                "Этот вариант для фиксации информации в первом квалификационном звонке с клиентом (Имя, должность, ГПР и так далее)\n"
                "\n"
                "<b>2. Анализ звонка в проигрыше:</b>\n"
                "Этот вариант для фиксации информации в проигрыше с клиентом и анализ разговора.\n"
                "\n"
                "<b>3. Общий анализ звонка:</b>\n"
                "Этот режим для резюмирования разговора, если нужно вытащить основную суть и зафиксировать всё тезисно\n"
                "\n"
                "<b>4. Проигрыш2 (тест)</b>\n"
                "Пока тестовый промт для анализа проигрыша руководителем.",
                reply_markup=get_analysis_keyboard(),
            ...
            await message.answer("Не удалось извлечь аудиодорожку.")
